...
class InferenceEngine: ObservableObject {
    ...
    // MARK: - Batch Processing Entry Point (Parallel Optimized Version)
    func runBatchPipeline(wavFolder: URL, accFolder: URL, gyroFolder: URL) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                // 4. Load models (load only once, shared across threads)
                // Note: CoreML prediction methods are usually thread-safe as long as no internal state is stored
                ...
                // Semaphore: limit concurrency to avoid memory spikes.
                // Recommended 4-6 for mobile, 8-12 can be tried on Mac (M4 Pro).
                // If set too high, CoreML resource contention might cause slowdowns or crashes.
                ...
                for fileSet in fileSets {
                    ...
                    DispatchQueue.global(qos: .userInteractive).async {
                        // ⚠️ Critical: Use autoreleasepool to ensure large memory chunks in each loop (audio buffer, MLMultiArray) are released promptly
                        autoreleasepool {
                            do {
                                ...
                                // print("✅ Completed: \(fileSet.id)") // Reduce log output to improve performance
                            } catch {
                                print("❌ Failed to process segment_\(fileSet.id): \(error.localizedDescription)")
                            }
                            
                            // Update progress (thread-safely updating UI)
                            self.lock.lock()
                            processedCount += 1
                            let currentCount = processedCount
                            self.lock.unlock()
                            
                            // Reduce UI refresh rate to avoid main thread freezing
    ...
    // MARK: - File Matching Logic (unchanged)
    ...
    // MARK: - Single Item Processing Logic (keep original logic, ensure no side effects)
    // This logic must be thread-safe (only use local variables, do not modify class member variables)
    ...
    // MARK: - Helper Functions (must be retained to support single processing)
    // Note: The following functions must be thread-safe (only operate on passed parameters and local variables)
