...
// Helper view remains unchanged
// Find FileSelectButton struct at the bottom of ContentView.swift
...
struct FileSelectButton: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack {
                // Fix: 'checkmark.folder.fill' does not exist, changed to 'folder.badge.plus' (unselected) and 'folder.fill' (selected)
                Image(systemName: selectedURL == nil ? "folder.badge.plus" : "folder.fill")
                    .foregroundColor(selectedURL == nil ? .blue : .green)
                    // If selected, a checkmark can be overlaid, or just distinguished by color
                ...
                // Add an extra checkmark icon to clarify status
                if selectedURL != nil {
